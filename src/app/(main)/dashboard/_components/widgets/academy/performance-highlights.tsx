...
export function PerformanceHighlights() {
  return (
    <Card className="h-full">
      <CardHeader>
        <CardTitle className="text-sm">Rendimiento destacado</CardTitle>
        <CardAction className="flex items-center gap-1 text-muted-foreground text-xs">
          Ver análisis <ArrowRight className="size-4" />
        </CardAction>
      </CardHeader>
      <CardContent>
        <ChartContainer config={chartConfig} className="h-70 w-full">
          ...
          >
            ...
            <XAxis
              axisLine={false}
              domain={[0, 4]}
              tickFormatter={(value) => ["Lun", "Mar", "Mié", "Jue", "Vie"][Number(value)] ?? ""}
              tickLine={false}
              tickMargin={10}
              ticks={[0, 1, 2, 3, 4]}
              type="number"
